...
export class OrdersService {
  ...
  async create(scheduleId: string, userId: string) {
    ...
    if (!mongoose.Types.ObjectId.isValid(userId)) throw new NotFoundException('Schedule not found!');
    ...
    await this.scheduleModel.findByIdAndUpdate(scheduleId, { $set: { user: userId } });
    await this.userModel.findByIdAndUpdate(userId, { $push: { orders: newOrder._id } });
    await this.userModel.findByIdAndUpdate(schedule.doctor._id, { $push: { orders: newOrder._id } });
    return { message: 'Schedule book successfully!', newOrder };
  }
  ...
  async approveOrder(orderId: string, userId: string) {
    ...
    if (!mongoose.Types.ObjectId.isValid(userId)) throw new BadRequestException('Order not found!');

    const order = await this.orderModel.findById(orderId)
    if (!order) throw new NotFoundException('Order not found!');
    if (String(order.doctor) !== String(userId)) throw new ForbiddenException('You are not allowed to approve this order!');
  ...
  async cancelOrder(orderId: string, userId: string) {
    return this._updateCancelStatus(orderId, userId, 'CANCELLED', 'USER');
  }
